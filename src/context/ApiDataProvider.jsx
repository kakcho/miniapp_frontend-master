...
import { ApiDataContext, TransparencyContext } from './ApiDataContext';
...
const ApiDataProvider = ({children}) => {
        ...
        const init = `TGMA ${window.Telegram.WebApp.initData}`
        const config = {
        headers: {
            Authorization:  window.Telegram.WebApp.initData ? init : testInit
        }}
        useEffect(() => {
          if (!data) {
        ...
        return (
          <ApiDataContext.Provider value={data}>
            <TransparencyContext.Provider value={{event, setEvent}}>
            {children}
